...
- [x] 4. Test and fix one-task-per-iteration enforcement (red/green TDD: write test showing multi-task completion goes undetected; then add post-iteration validator in `ralph-loop.sh` comparing plan before/after, fail closed on violation) — **coder**
- [x] 5. Final review of all 4 fixes — verify tests pass, check for regressions, confirm acceptance criteria (depends: 1,2,3,4) — **critic**
