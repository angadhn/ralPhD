...
./ralph plan                  # plan (uses launcher)
./ralph -p build              # headless build
```

The init script creates directories, symlinks `specs/` and `templates/` back to the framework, copies starter files, and generates a `./ralph` launcher.
...
```
my-paper/
├── ralph                       # Launcher (generated by init-project.sh)
...
Resolution order: `RALPH_HOME` env var > `.ralphrc` file (read by `./ralph` launcher) > script directory (default, backward compatible).
...
steps:
  ...
  - run: RALPH_HOME=$GITHUB_WORKSPACE/.ralph-engine ./ralph -p build 5
```
