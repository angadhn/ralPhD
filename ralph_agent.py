...
Tools live in tools.py (schema + handler colocated per tool, à la ghuntley's
coding-agent pattern). This file is the orchestrator: auth, loop, CLI.
...
from tools import TOOLS, AGENT_TOOLS, DEFAULT_TOOLS, api_schema, execute_tool
...
def run_agent(agent_name: str, system_prompt: str, task: str, model: str,
    ...
    tools = [api_schema(TOOLS[t]) for t in tool_names]
