...
### Layout A: all-in-one workspace (Quick Start B)

All content and framework state live together in one directory. This is the
default layout when the workspace argument is not named `.ralph`.

```
my-paper/                               ← WORKSPACE = PROJECT_ROOT (cwd when running)
...
├── templates/      → $RALPH_HOME/templates     ← symlink (self-healing)
├── .claude/agents/ → $RALPH_HOME/.claude/agents ← symlink (self-healing)
├── inputs/         → human-inputs              ← symlink (backward compat)
├── human-inputs/                       ← user-provided context
├── ai-generated-outputs/               ← AI outputs, organized by thread
├── papers/                             ← input PDFs
...
### Layout B: split layout — content at root, framework state in `.ralph/` (Quick Start C)

Used for brownfield projects (workspace named `.ralph/`). Content directories
live at the **project root**, outside the framework workspace. Disposable
framework state lives inside `.ralph/`. Symlinks inside `.ralph/` point to the
project-root directories, so agents see everything via relative paths from cwd
— **zero changes to agent prompts, tools, or specs**.

```
myproject/                              ← PROJECT_ROOT (cwd when running)
├── human-inputs/
...
│   ├── .claude/agents/ → $RALPH_HOME/.claude/agents
│   ├── inputs/         → ../human-inputs       ← symlink (backward compat)
