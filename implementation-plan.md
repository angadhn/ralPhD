...
- [ ] 2. Test and fix terminal checkpoint dispatch (red/green TDD: write failing test showing prose like "Thread ready for review" extracts bogus agent; then fix `detect.sh` to use `<all tasks complete>` as canonical done marker, expand the case guard at line 25) — **coder**
- [ ] 3. Test and fix DOI-to-BibTeX fail-open (red/green TDD: write tests for missing bibtexparser and missing .bib; then fix `_citation.py` to raise explicit errors, update `verify.py` to surface them) — **coder**
